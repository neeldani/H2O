...
def apply_rotary_pos_emb_single(x, cos, sin, position_ids):
    ...
    bsz, num_heads, seq_len, dim = x.size()
    cos = cos.squeeze(1).squeeze(0)  # [seq_len, dim]
    sin = sin.squeeze(1).squeeze(0)  # [seq_len, dim]
    if len(position_ids.shape) == 2:
        position_ids = position_ids[0]  # each sequence in the batch has the same position_ids
    
    cos = cos[position_ids]  # [bs, 1, seq_len, dim]
    sin = sin[position_ids]  # [bs, 1, seq_len, dim]
    cos = cos.expand(bsz, num_heads, seq_len, dim)
    sin = sin.expand(bsz, num_heads, seq_len, dim)
...
class H2OKVCache_LayerWise:
    ...
    def _update_hh_score(self, attn_score_cache):
        ...
        if self.hh_score is None:
            self.hh_score = attn_score_cache.sum(2) # (bsz, num_heads, 1, head_dim)
        else:
...
class H2OLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            self.hh_size = int(self.config.heavy_ratio * kv_seq_len)
            self.recent_size = int(self.config.recent_ratio * kv_seq_len)
            self.cache_budget = self.hh_size + self.recent_size
            
            # init the kv cache here
        ...
        position_length = kv_seq_len # q_len
        if torch.all(position_ids == position_ids[0][0]):
            if position_length < position_ids[0].item()+1:
